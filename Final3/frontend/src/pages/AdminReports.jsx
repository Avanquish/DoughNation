...
export default function AdminReports() {
  ...
  const renderAdminHeader = () => {
    ...
    return (
      <div className="mb-6 p-4 bg-gradient-to-r from-[#FFF3E6] via-[#FFE1BD] to-[#FFD199] rounded-xl shadow-md flex items-center gap-4">
        <img
        ...
        />
        <div>
          <p className="text-sm text-[#6b4b2b]/70 font-medium">Report Generated By</p>
          <p className="text-lg font-bold text-[#6b4b2b]">Administrator</p>
          <p className="text-xs text-[#7b5836]">{new Date().toLocaleDateString('en-PH', { 
            year: 'numeric', 
            month: 'long', 
            day: 'numeric' 
          })}</p>
  ...
  const downloadReportPDF = async () => {
    ...
    if (adminProfile && adminProfile.profile_picture) {
      try {
        ...
        if (adminImgBase64) {
          // Calculate image dimensions to match print style
          const maxWidth = 350; // Match your print width
          const imgWidth = Math.min(maxWidth, pageWidth * 0.9);
          
          // Get actual image dimensions to maintain aspect ratio
          const img = new Image();
          img.src = adminImgBase64;
          ...
          doc.addImage(adminImgBase64, "JPEG", imgX, currentY, imgWidth, imgHeight);
  ...
  const printReport = () => {
    ...
    const adminImageHTML = adminProfile && adminProfile.profile_picture 
          ? `<div style="text-align: center; margin-bottom: 20px;">
              ...
              <p style="margin: 5px 0; font-size: 12px; color: black;"> Generated By: Administrator</p>
              <p style="margin: 0; font-size: 11px; color: black;">${new Date().toLocaleDateString()}</p>
            </div>`
          : '';
...
const downloadDonationPDF = async () => {
  ...
  doc.setFontSize(16);
  doc.setFont("helvetica", "bold");
  doc.text("DONATION LIST REPORT", pageWidth / 2, currentY, { align: "center" });
  currentY += 20;

  // Date range
  doc.setFontSize(10);
  doc.setFont("helvetica", "normal");
  doc.text(`Period: ${startDate} to ${endDate}`, pageWidth / 2, currentY, { align: "center" });
  currentY += 25;
