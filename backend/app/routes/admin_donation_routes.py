...
def update_admin_donation_tracking(
    ...
    if current_user.role == "Donor":
        ...
        if old_status == "in_transit" and request.tracking_status not in ["received"]:
            raise HTTPException(status_code=400, detail="Can only move from 'in_transit' to 'received'")
    ...
    # If status is received, add to admin inventory (if not already added)
    if request.tracking_status == "received" and old_status != "received" and old_status != "complete":
        ...
        product_id = generate_product_id(db, donation.donation_name, admin_user.id)
        ...
        admin_inventory_item = models.AdminInventory(
            ...
            donation_request_id=None,  # Set to None as this is from admin_donation_requests, not donation_requests
