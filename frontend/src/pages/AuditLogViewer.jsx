...
useEffect(() => {
  ...
  const fetchLogs = async () => {
    try {
      ...
      if (eventTypeFilter === "admin_crud") {
        const adminCrudTypes = ["ADMIN_UPDATE_USER", "ADMIN_MANUAL_REGISTRATION", "ADMIN_DELETE_USER"];
        filteredLogs = filteredLogs.filter(log => adminCrudTypes.includes(log.event_type));
      }
  ...
  try {
    ...
    if (eventTypeFilter === "admin_crud") {
      const adminCrudTypes = ["ADMIN_UPDATE_USER", "ADMIN_MANUAL_REGISTRATION", "ADMIN_DELETE_USER"];
      allLogs = allLogs.filter(log => adminCrudTypes.includes(log.event_type));
    }

    if (allLogs.length === 0) {
      Swal.fire("Warning", "No logs found matching the current filters", "warning");
      return;
    }
  ...
  doc.text("AUDIT LOG REPORT", pageWidth / 2, currentY, { align: "center" });
  currentY += 18;
  ...
  if (startDate || endDate || eventTypeFilter || severityFilter) {
    ...
    if (eventTypeFilter && eventTypeFilter !== "all") filterText += `Event: ${eventTypeFilter} `;
    if (severityFilter && severityFilter !== "all") filterText += `Severity: ${severityFilter}`;
  ...
  // Table
  const headers = ["Timestamp", "Event Type", "Actor", "Severity", "Status", "Description"];
  ...
  autoTable(doc, {
      ...
      columnStyles: {
        0: { cellWidth: availableWidth * 0.115, halign: "center", font: "helvetica" }, // Timestamp
        1: { cellWidth: availableWidth * 0.14, halign: "center", font: "helvetica" },  // Event Type
        2: { cellWidth: availableWidth * 0.11, halign: "center", font: "helvetica" },  // Actor
        3: { cellWidth: availableWidth * 0.085, halign: "center", font: "helvetica" }, // Severity
        4: { cellWidth: availableWidth * 0.085, halign: "center", font: "helvetica" }, // Status
        5: { cellWidth: availableWidth * 0.465, halign: "center", font: "helvetica" }  // Description
      },
  ...
  Swal.fire("Success", "Audit logs exported as PDF successfully", "success");
  } catch (error) {
