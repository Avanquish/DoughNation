...
const sendMessage = () => {
  ...
  try {
    // ✅ Update backend inventory status to "requested"
    await axios.post(
      `${API_URL}/donations/confirm-request`,
      { 
        donation_id: donation.id, 
        charity_id: originalCharityId,
        bakery_id: currentUser.id
      },
      { headers }
    );
    ...
    // ✅ Notify backend via WebSocket (optional, if you keep this flow)
    wsRef.current?.send(
      JSON.stringify({
        type: "accept_donation",
        id: donationCardMessage.id,
        donation_id: donation.id,
      })
    );

    // ✅ Update local state immediately (UI reflects without refresh)
    setMessages((prev) =>
      prev.map((m) => {
        if (m.id === donationCardMessage.id) {
          return {
            ...m,
    ...
    // ✅ Store accepted donations (optional)
